...
class ChangeOrderDeliveryDateTest extends TestCase
{
    ...
    public function it_should_change_the_delivery_date()
    {
        ...
        $url = route('change-order-delivery-date', $order->reference());
        $deliveryDate = CarbonImmutable::now()->next(CarbonInterface::MONDAY);
        ...
        $foundOrder->picklists()->each(function (Picklist $picklist) use ($deliveryDate) {
            self::assertEquals($picklist->preferredDeliveryDate(), $deliveryDate);
        });
    }
}
